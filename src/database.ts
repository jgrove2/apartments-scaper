...
export function uploadApartmentsToDB(apartmentData: {$url: string, $price: number, $bedrooms: number, $bathrooms: number}[]){
...
export function deleteAllApartmentsNotExisting(apartmentData: {$url: string, $price: number, $bedrooms: number, $bathrooms: number}[]){
    console.log("[deleteAllApartmentsNotExisting]: Starting function...")
    var db = new Database('./db/apartments.db');

    db.run(`DELETE FROM apartments WHERE url NOT IN (${apartmentData.map((data) => `\'${data.$url}\'`).join(', ')})`);
    db.close(false);
    console.log("[deleteAllApartmentsNotExisting]: Ending function...")
}

export function updateAllOldApartmentsToOld(apartmentData: {$url: string, $price: number, $bedrooms: number, $bathrooms: number}[]){
...
export function insertAllNewApartments(apartmentData: {$url: string, $price: number, $bedrooms: number, $bathrooms: number}[]){
